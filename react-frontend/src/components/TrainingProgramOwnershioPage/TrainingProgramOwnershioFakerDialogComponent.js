...
import trainingProgramOwnershioFakerFactory from "./trainingProgramOwnershioFakerFactory";
...
const TrainingProgramOwnershioFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = trainingProgramOwnershioFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("trainingProgramOwnershio").create(elem));
...
export default connect(mapState, mapDispatch)(TrainingProgramOwnershioFakerDialogComponent);
