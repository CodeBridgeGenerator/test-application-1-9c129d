...
import seederData from "./trainingProgramOwnershio.seeder.json";
...
const TrainingProgramOwnershioSeederDialogComponent = (props) => {
    ...
    const onRun = async () => {
        ...
        try {
            for (let i = 0; i < seederData.length; i++) {
                ...
                const res = await client.service("trainingProgramOwnershio").create(_data);
                results = [...results, res];
            }
            props.onHide();
...
export default connect(mapState, mapDispatch)(TrainingProgramOwnershioSeederDialogComponent);
