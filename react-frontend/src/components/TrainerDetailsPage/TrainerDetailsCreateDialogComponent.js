...
const TrainerDetailsCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            trainerIcNo: _entity?.trainerIcNo,organizationName: _entity?.organizationName,employmentType: _entity?.employmentType,nationality: _entity?.nationality,
    ...
    return (
        <Dialog header="Create Trainer Details" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max" footer={renderFooter()} resizable={false}>
            ...
            <span className="align-items-center">
                <label htmlFor="trainerIcNo">trainer_IC_No:</label>
                <InputText id="trainerIcNo" className="w-full mb-3 p-inputtext-sm" value={_entity?.trainerIcNo} onChange={(e) => setValByKey("trainerIcNo", e.target.value)}  />
            </span>
            <small className="p-error">
            {!_.isEmpty(error["trainerIcNo"]) ? (
              <p className="m-0" key="error-trainerIcNo">
                {error["trainerIcNo"]}
              </p>
            ) : null}
          </small>
...
<div className="col-12 md:col-6 field mt-5">
            <span className="align-items-center">
                <label htmlFor="employmentType">employment_type:</label>
